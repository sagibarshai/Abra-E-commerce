...
export default (props) => {
     ...
     const deleteItemHandler = (productObj) => {
          let exsistingItem = props.itemsInCart.find(
               (item) => item.name === productObj.name
          );
          let updatedItemsInCart;
          if (exsistingItem) {
               updatedItemsInCart = [...props.itemsInCart];
               updatedItemsInCart = updatedItemsInCart.filter((item) => {
                    return item.name !== productObj.name;
               });
               props.setItemsInCart(updatedItemsInCart);
          }
          let updatedPrice = Number(
               props.totalPrice - productObj.cartQty * productObj.price
          );
          props.setTotalPrice(updatedPrice);
          let updatedProductObj = { ...productObj, cartQty: 0 };
          let data = { updatedProductObj, updatedPrice, updatedItemsInCart };
          axios.put(
               process.env.REACT_APP_BACKEND_URL +
                    `/users/${props.userId}/delete`,
               data
          )
               .then((res) => {
                    props.setTotalPrice(res.data.totalPrice);
     ...
     const decreseQtyByOneHandler = (productObj) => {
          ...
          if (exitingItem.cartQty === 1) {
               ...
               props.setItemsInCart(updatedItemsInCart);
               updatedPrice =
                    totalPrice - updatedItemsInCart[exitingItemIndex].price;
               props.setTotalPrice(updatedPrice);
          }
          axios.put(
               process.env.REACT_APP_BACKEND_URL +
                    `/users/${props.userId}/decrese`,
               {
