...
const AddProduct = (props) => {
  ...
  const onSubmitHandler = (e) => {
    ...
    let errorsArray = []
    e.preventDefault()
    if (name.length < 2) errorsArray.push(`Name filed must contain at least 2 characters`)
    if (imgSrc.length <= 10) errorsArray.push(`Image src filed must contain at least 10 characters`)
    if (price <= 0 || !price) errorsArray.push(`Price filed must be above 0 !`)
    if (quantity <= 0 || !quantity) errorsArray.push(`Quantity field must be above 0 !`)
    if (types.length <= 0) errorsArray.push(`You must choose at least 1 type`)
    setErrors(errorsArray)
    if (errorsArray.length === 0) {
      const data = { name, price, imgSrc, quantity, types }
      axios.put(process.env.REACT_APP_BACKEND_URL + '/manager/add-product', data)
        .then(res => {
          props.setAllProduts(res.data.items)
